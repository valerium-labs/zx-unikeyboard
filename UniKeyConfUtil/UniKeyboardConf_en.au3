...
Const $maintitle = "ZX Unikeyboard configuration tool"
Const $settitle = "COM port setting"
...
$BtnSetPort = GUICtrlCreateButton("Set COM-port", 16, 312, 105, 30, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
$BtnAbout = GUICtrlCreateButton("About", 131, 312, 105, 30, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
$BtnEditTemplate = GUICtrlCreateButton("Edit keyboard template", 16, 20, 220, 30, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
$BtnOpenTemplate = GUICtrlCreateButton("Open keyboard template", 16, 60, 220, 30, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
$BtnStartReading = GUICtrlCreateButton("Start reading keys", 16, 100, 220, 30, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
GUICtrlSetState(-1, $GUI_DISABLE)
$LabelStart = GUICtrlCreateLabel("Press a key for:", 26, 140, 200, 30)
...
GUICtrlSetState(-1, $GUI_HIDE)
$LabelRead = GUICtrlCreateLabel("Keys read: ", 26, 200, 60, 30)
GUICtrlSetColor(-1, 0x000000)
GUICtrlSetState(-1, $GUI_HIDE)
$KeysRead = GUICtrlCreateLabel("", 90, 200, 40, 30)
GUICtrlSetColor(-1, 0x000000)
GUICtrlSetState(-1, $GUI_HIDE)
$LabelLeft = GUICtrlCreateLabel("Keys left: ", 130, 200, 60, 30)
GUICtrlSetColor(-1, 0x000000)
GUICtrlSetState(-1, $GUI_HIDE)
$KeysLeft = GUICtrlCreateLabel("", 190, 200, 40, 30)
GUICtrlSetColor(-1, 0x000000)
GUICtrlSetState(-1, $GUI_HIDE)
$BtnAnalyze = GUICtrlCreateButton("Analyze", 16, 240, 220, 50, $BS_FLAT)
GUICtrlSetColor(-1, 0x008000)
GUICtrlSetState(-1, $GUI_DISABLE)
GUISetState(@SW_SHOW)
; MAIN FORM *******************************************************************************
...
While 1
    ...
    if ($nNextKey = UBound($aKeyArray)) and (UBound($aKeyArray) > 0) Then
         ...
         $nNextKey = 0
         If MsgBox($MB_SYSTEMMODAL+$MB_OKCANCEL+$MB_ICONQUESTION, "Success", UBound ($aKeyArray) & " keys have been read successfully." & @CRLF _
             & "Would you like to analyze the collected data ?") = $IDOK then AnalyzeEvent ()  
         $nStartReading = 0
    endIf
...
Func AboutEvent()
          MsgBox($MB_SYSTEMMODAL + $MB_ICONINFORMATION, "About this program", "ZX UniKeyboard Configuration Utility" & @CRLF & @CRLF & _
...
Func OpenFileEvent()
    GUICtrlSetState($BtnStartReading, $GUI_DISABLE)             
    $sFile = FileOpenDialog("Choose template file...", ".\Template", "(*.txt)")
    if $sFile <> '' Then
	$aKeyArray = FileReadToArray($sFile)
	If UBound ($aKeyArray)>0 Then
            MsgBox($MB_SYSTEMMODAL, "Load OK", UBound ($aKeyArray) & " keys loaded")
...
Func EditFileEvent()
    $sFile = FileOpenDialog("Choose template file to edit...", ".\Template", "(*.txt)")
    if $sFile <> '' Then
...
; _ArrayDisplay ($nRows)
    ...
    next
    if $unbound <> "" then  MsgBox(BitOR ($MB_SYSTEMMODAL, $MB_ICONWARNING), "Warning !", "Some unbound keylines left: "& @CRLF & $unbound & @CRLF & _
               "It happens if there's a column and row " & @CRLF & "that belongs to only one key" & _ 
               @CRLF & "or if there was a mistake in a keyboard template" & @CRLF & @CRLF &"You have to check these line(s) manually.")


    $sFile = FileSaveDialog("Choose a folder to save header file...", @ScriptDir, "(*.h)", 16, "customkey.h")
    if @error then 
         MsgBox(BitOR ($MB_SYSTEMMODAL, $MB_ICONWARNING),  "Warning !", 'File was not saved. Press "Analyze" button to analyze and save the file again')
    else
    ...
    GUICtrlSetState($BtnAnalyze, $GUI_DISABLE)
    MsgBox(BitOR ($MB_SYSTEMMODAL, $MB_ICONINFORMATION), "Congratulations", "Headerfile successfully created" )
    endif
EndFunc   ;==>Analyze
