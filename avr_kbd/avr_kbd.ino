/*
 * This is and AVR firmware source code for ZX Unikeyboard controller.
            ...
            if (!digitalRead(pins[rows[rowcount]])) 
             {
                ...
                for (i=0; i<sizeof(keyaddr)/3; i++)
                  if ( ((keyaddr[i][1]==cols[colcount]) and (keyaddr[i][2]==rows[rowcount])) or
                      ((keyaddr[i][2]==cols[colcount]) and (keyaddr[i][1]==rows[rowcount])) )
                  {
                    #if DEBUG_MODE
